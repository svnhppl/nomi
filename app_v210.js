const STORAGE_KEY="nomiSettingsV210";
...
const audioFiles={inhale:"./einatmen.mp3?v=210",exhale:"./ausatmen.mp3?v=210",end:"./ende.mp3?v=210"};
...
function startSession(){
  document.body.innerHTML=`<main id="session"><div id="timer">${formatTime(settings.duration*60)}</div><div id="phaseLabel"></div><div id="flowerStage" aria-hidden="true"><div id="breathFlower">${Array.from({length:8}).map((_,index)=>`<div class="breath-shape" data-index="${index}">${shapeMarkup(index)}</div>`).join("")}</div></div><div id="countdown"><div id="countdownLabel">Prepare</div><div id="countdownBar" aria-hidden="true"><div id="countdownBarFill"></div></div></div><button id="exitButton" type="button">End</button><div id="endMessage">Completed</div></main>`;
  applyTheme(settings.theme);
...
function startCountdown(seconds){
  ...
  function animatePrepare(now){
    const progress=Math.min(1,(now-startedAt)/durationMs);
    if(countdownBarFill)countdownBarFill.style.width=`${(progress*100).toFixed(2)}%`;
    if(progress<1){
      requestAnimationFrame(animatePrepare);
      return;
    }
    countdownElement.classList.add("is-complete");
    setTimeout(()=>{
