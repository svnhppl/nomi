const STORAGE_KEY="nomiSettingsV211";
...
const audioFiles={inhale:"./einatmen.mp3?v=211",exhale:"./ausatmen.mp3?v=211",end:"./ende.mp3?v=211"};
...
function startSession(){
  document.body.innerHTML=`<main id="session"><div id="timer"><div id="sessionProgress" aria-label="Remaining time"><div id="sessionProgressFill"></div></div></div><div id="phaseLabel"></div><div id="flowerStage" aria-hidden="true"><div id="breathFlower">${Array.from({length:8}).map((_,index)=>`<div class="breath-shape" data-index="${index}">${shapeMarkup(index)}</div>`).join("")}</div></div><div id="countdown"><div id="countdownLabel">Prepare</div><div id="countdownNumber">3</div></div><button id="exitButton" type="button">End</button><div id="endMessage">Completed</div></main>`;
  applyTheme(settings.theme);
