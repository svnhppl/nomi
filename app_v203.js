const STORAGE_KEY="nomiSettingsV203";
...
const audioFiles={inhale:"./einatmen.mp3?v=203",exhale:"./ausatmen.mp3?v=203",end:"./ende.mp3?v=203"};
...
function loadSettings(){try{return {...DEFAULTS,...JSON.parse(localStorage.getItem(STORAGE_KEY)||"{}")}}catch(error){return {...DEFAULTS}}}
...
function sanitizeSeconds(value,fallback){const parsed=Number.parseFloat(String(value).replace(",","."));if(!Number.isFinite(parsed))return fallback;return Math.min(8,Math.max(4,Math.round(parsed*2)/2))}
function saveSettingsFromUi(){
  const selectedTheme=document.querySelector('input[name="theme"]:checked');
  const selectedDuration=document.querySelector('input[name="duration"]:checked');
  settings={theme:selectedTheme?selectedTheme.value:"ocean",duration:selectedDuration?Number(selectedDuration.value):1,inhale:sanitizeSeconds(inhaleInput.value,4.5),exhale:sanitizeSeconds(exhaleInput.value,6.5),showTimer:showTimerToggle.checked,showPhaseText:showPhaseTextToggle.checked,sound:soundToggle.checked,fullscreen:fullscreenToggle.checked};
  persistSettings();
  applySettingsToUi();
  applyTheme(settings.theme);
  closeSettings();
...
async function unlockAudioElements(){
  ...
  for(const key of Object.keys(audioElements)){
    ...
    try{
      ...
      try{audio.muted=false;audio.pause();audio.currentTime=0}catch(e){}
    }
  }
}
...
function startBreathingAnimation(){
  ...
  function animate(now){
    ...
    if(position<=settings.inhale){phase="inhale";phaseDuration=settings.inhale;breathValue=easeInOutSine(position/settings.inhale)}
    else{phase="exhale";phaseDuration=settings.exhale;breathValue=1-easeInOutSine((position-settings.inhale)/settings.exhale)}
