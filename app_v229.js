const STORAGE_KEY = "nomiSettingsV229";
const DEFAULTS = {
  duration: 1,
  rhythm: "longerExhale",
  showTimer: true,
...
const audioFiles = {
  inhale: "./einatmen.mp3?v=229",
  exhale: "./ausatmen.mp3?v=229",
  end: "./ende.mp3?v=229"
};
...
function sessionShapesMarkup() {
  return Array.from({ length: 8 }).map((_, i) => `<div class="breath-shape" data-index="${i}">${shapeMarkup()}</div>`).join("");
}
...
function startBreathingAnimation() {
  ...
  function animate(now) {
    ...
    const elapsed = (now - sessionStartTime) / 1000,
          cycle = rhythm.inhale + rhythm.exhale,
          pos = elapsed % cycle;
...
function updateFlower(breathValue) {
  ...
  shapes.forEach((shape, index) => {
    const count = shapes.length,
          ...
          isOrbs = settings.geometry === "orbs",
          radial = 5 + phase * 28,
          sx = 1.02 + phase * .14,
          sy = 1.02 + phase * .14,
          localAngle = isOrbs ? angle : (angle + (phase - .5) * 2.0 * (index % 2 === 0 ? 1 : -1)),
          opacity = isOrbs ? (.42 + phase * .16) : (.42 + phase * .14);
...
function playPersistentAudio(key, targetDuration, onEnded) {
  ...
  try {
    ...
    if (Number.isFinite(a.duration) && a.duration > 0 && targetDuration) {
      a.playbackRate = Math.min(4, Math.max(.25, a.duration / targetDuration));
    } else {
      a.playbackRate = 1;
    }
    ...
    // Fallback safety timeout if audio onended does not fire
    if (onEnded) {
      setTimeout(() => triggerEnd(), 3200);
    }
  } catch (e) {
    triggerEnd();
  }
...
function finishSession() {
  ...
  const session = document.getElementById("session"),
        label = document.getElementById("phaseLabel");
